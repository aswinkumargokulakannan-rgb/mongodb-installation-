...
###############################################################################
# MongoDB Configuration and Security Setup Script (v3 - Multi-version support)
# Configures MongoDB for remote access from Application Server (192.168.0.10)
# Creates 'vcard' database with authentication
# Supports both MongoDB 4.4 (mongo) and 7.0+ (mongosh)
###############################################################################

set -e

echo "=========================================="
echo "MongoDB Configuration Script v3"
echo "=========================================="
echo ""
...
# Find MongoDB shell binary (mongosh for 5.0+, mongo for 4.4 and earlier)
MONGO_SHELL=""
if command -v mongosh &> /dev/null; then
    ...
    MONGO_SHELL="mongo"
    echo "Detected: MongoDB 4.4 or earlier (using mongo)"
elif [ -f /usr/bin/mongosh ]; then
    MONGO_SHELL="/usr/bin/mongosh"
    echo "Using: /usr/bin/mongosh"
elif [ -f /usr/bin/mongo ]; then
    MONGO_SHELL="/usr/bin/mongo"
    echo "Using: /usr/bin/mongo"
else
    echo "ERROR: No MongoDB shell found (mongosh or mongo)!"
    echo "Please run: sudo ./fix-cpu-compatibility.sh first"
    exit 1
fi
...
MONGO_VERSION=$($MONGO_SHELL --version 2>&1 | head -n 1)
...
# Security - disabled initially to create users
#security:
...
# Additional wait for MongoDB to be ready to accept connections
sleep 5

echo "Step 5: Creating MongoDB admin user..."

# Different syntax for mongo vs mongosh
if [ "$MONGO_SHELL" = "mongo" ]; then
    # MongoDB 4.4 and earlier use mongo shell
    $MONGO_SHELL --quiet admin <<EOF
db.createUser({
  user: "admin",
  pwd: "${DB_PASSWORD}",
  roles: [
...
# Test connection
if [ "$MONGO_SHELL" = "mongo" ]; then
    $MONGO_SHELL "mongodb://${DB_USER}:${DB_PASSWORD}@${DB_SERVER_IP}:27017/${DB_NAME}" --quiet --eval "db.runCommand({connectionStatus: 1})" > /dev/null
else
    $MONGO_SHELL "mongodb://${DB_USER}:${DB_PASSWORD}@${DB_SERVER_IP}:27017/${DB_NAME}" --quiet --eval "db.runCommand({connectionStatus: 1})" > /dev/null
fi
