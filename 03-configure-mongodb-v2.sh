...
###############################################################################
# MongoDB Configuration and Security Setup Script (v2 - PATH fixed)
...
echo "=========================================="
echo "MongoDB Configuration Script v2"
echo "=========================================="
echo ""
...
elif [ -f /usr/local/bin/mongosh ]; then
    MONGOSH_BIN="/usr/local/bin/mongosh"
else
    # Try to find it
    MONGOSH_BIN=$(find /usr -name mongosh -type f 2>/dev/null | head -n 1)
fi

if [ -z "$MONGOSH_BIN" ]; then
    echo "ERROR: mongosh not found!"
    echo "Please run: sudo ./quick-fix-install.sh first"
    exit 1
fi

echo "Using mongosh: $MONGOSH_BIN"
echo ""
...
# First, update the config to disable auth temporarily
cat > /etc/mongod.conf <<EOF
# mongod.conf - MongoDB Configuration File (Temporary - No Auth)
...
$MONGOSH_BIN "mongodb://${DB_USER}:${DB_PASSWORD}@${DB_SERVER_IP}:27017/${DB_NAME}" --quiet --eval "db.runCommand({connectionStatus: 1})" > /dev/null
